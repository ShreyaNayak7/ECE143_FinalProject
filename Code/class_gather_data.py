...
class DataHandler:
    ...
    def gather_data_from_Excel(path_of_file, sh_name,index='default',header=0,fillnan='no_fill'):
        ...
        Inputs: path_of_file 
                (string)
                The path of the excel file that contains data
                
                sh_name 
                (string) 
                name of the sheet that has the data to load
                
                index
                (str)
                Colum name that needs to be set as index
                'default' implies no specific column to be set as the index
                
                header
                (index)
                The row from which the dataframe needs to start
                
                fillnan
                (str/int)
                specifies the value that will replace nan in the dataframe
        ...
        assert (isinstance(fillnan,str) or isinstance(fillnan,int))
